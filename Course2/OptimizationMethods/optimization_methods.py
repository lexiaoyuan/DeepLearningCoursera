...
def random_mini_batches(X, Y, mini_batch_size=64, seed=0):
    ...
    if m % mini_batch_size != 0:
        ...
        mini_batch_X = shuffled_X[:,
                                  mini_batch_size * num_complete_minibatches:]
...
def model(X, Y, layers_dims, optimizer, learning_rate=0.0007, mini_batch_size=64, beta=0.9, beta1=0.9, beta2=0.999, epsilon=1e-8, num_epochs=10000, print_cost=True, is_plot=True):
    ...
    for i in range(num_epochs):
        # 定义随机minibatches，我们在每次遍历数据集后增加种子以重新排列数据集，使每次数据的顺序都不同
        seed = seed + 1
...
# 使用动量的梯度下降
layers_dims = [train_X.shape[0], 5, 2, 1]
parameters = model(train_X, train_Y, layers_dims, beta=0.9,
                   optimizer="momentum", is_plot=True)
