...
plt.rcParams['figure.figsize'] = (7.0, 4.0)  # 设置plots默认大小
...
def model(X, Y, learning_rate=0.3, num_iterations=30000, print_cost=True, is_plot=True, lambd=0, keep_prob=1):
    ...
    for i in range(0, num_iterations):
        ...
        # 反向传播
        # 可以同时使用L2正则化和随机删除结点，但是本次实验不同时使用
        ...
        # 更新参数
        parameters = reg_utils.update_parameters(
...
def compute_cost_with_regularization(A3, Y, parameters, lambd):
    ...
    L2_regularization_cost = lambd * \
        (np.sum(np.square(W1)) + np.sum(np.square(W2)) + np.sum(np.square(W3))) / (2*m)
...
def forward_propagation_with_dropout(X, parameters, keep_prob=0.5):
    ...
    # Linear -> ReLU -> Linear -> ReLU -> Linear ->Sigmod
    Z1 = np.dot(W1, X) + b1
...
def backward_propagation_with_dropout(X, Y, cache, keep_prob):
    ...
    dA2 = dA2 * D2  # 步骤1：使用正向传播期间相同的结点，舍弃那些关闭的结点
    dA2 = dA2 / keep_prob  # 步骤2：缩放未舍弃的结点（不为0）的值
    ...
    dA1 = dA1 * D1  # 步骤1：使用正向传播期间相同的结点，舍弃那些关闭的结点
    dA1 = dA1 / keep_prob  # 步骤2：缩放未舍弃的结点（不为0）的值
...
parameters = model(train_X, train_Y, keep_prob=0.86,
                   learning_rate=0.3, is_plot=True)
print("使用Dropout正则化，训练集：")
predictions_train = reg_utils.predict(train_X, train_Y, parameters)
print("使用Dropout正则化，测试集：")
predictions_test = reg_utils.predict(test_X, test_Y, parameters)

# 绘制分割曲线
plt.rcParams['font.sans-serif'] = ['SimHei']
plt.rcParams['axes.unicode_minus'] = False
plt.title("使用Dropout正则化")
ax = plt.gca()
ax.set_xlim([-0.75, 0.40])
ax.set_ylim([-0.75, 0.65])
reg_utils.plot_decision_boundary(
